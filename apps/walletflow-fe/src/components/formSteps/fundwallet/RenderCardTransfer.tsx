...
const RenderCardTransfer = ({form,isFormSubmitted,openDialog}:{form:any,isFormSubmitted?:boolean,openDialog:(content:ReactNode)=>void}) => {
  return (
        <div>
                    ...
                    {/* submit button */}
                    <Button className='w-full' onClick={() => openDialog(<SuccessDialog text="your wallet was funded successfully" logo="./successLogo.png" submitText="Done" />)}>{isFormSubmitted ? <LoaderCircle /> : "Fund"}</Button>
                </div>
  )
}

export default RenderCardTransfer